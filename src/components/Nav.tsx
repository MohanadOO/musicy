    ...
    <header className='px-5 sm:px-10 border-b-2 border-transparent'>
      <nav className='py-5' aria-label='Primary'>
        <ul className='flex items-center gap-3 sm:gap-6 child:cursor-pointer z-20 child:font-bold child:uppercase lg:text-sm xl:text-base 2xl:text-lg'>
                ...
                <>
                  <Menu.Button className='uppercase underline truncate'>
                    <div className='flex gap-1'>
                      {t('forArtist')}
                      <HiChevronDown
                        className={`w-5 h-5 transition-transform my-auto ${
                          open ? 'rotate-180' : ''
                        }`}
                      />
                    </div>
          ...
          <li className='hidden text-xs xl:text-base 2xl:text-lg lg:block'>
            <a href='#'>{t('login')}</a>
          </li>
          <li className='hidden text-xs xl:text-base 2xl:text-lg lg:flex items-center justify-center text-center bg-black text-white py-3 px-3 xl:px-7 rounded-full'>
            <a href='#'>{t('makeYourOwn')}</a>
          </li>
          <li>
            {Object.keys(languages).map((lan) =>
              i18n.resolvedLanguage !== lan ? (
                <button
